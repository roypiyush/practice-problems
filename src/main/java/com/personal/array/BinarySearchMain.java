...
public class BinarySearchMain {
    ...
    int firstElementGreaterThanOrEqual(int arr[], int key) {
        int i = 0;
        int j = arr.length;

        while (i < j) {
            int m = i + (j - i) / 2;
            if (arr[m] < key) { // here, it ensures that anything < key will be skipped with +1
                i = m + 1;
            } else {
                // finds first element duplicates occur
                j = m;
            }
        }
    ...
    int firstElementGreaterThan(int arr[], int key) {
        int i = 0;
        int j = arr.length;
        ...
        // element always greater than key
        return i;
    }

    int firstElementLessThanOrEqual(int arr[], int key) {
        int i = 0;
        int j = arr.length;
        int result = -1;
